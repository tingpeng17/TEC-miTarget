...
def main(args):
    ...
    try:
        ...
        sys.exit(1)
    # Load Pairs
    dataset = make_input_pair(args.mirna_file, args.mrna_file, args.query_file, cts_size=30, seed_match='custom', header=True, train=False) # dict
    ...
    with open(outPathAll, "w+") as f:
        f.write('query_ids\tquery_seqs\ttarget_ids\ttarget_seqs\ttarget_locs\tpredictions\n')
        with open(outPathPos, "w+") as pos_f:
            pos_f.write('query_ids\tquery_seqs\ttarget_ids\ttarget_seqs\ttarget_locs\tpredictions\n')
            ...
            with torch.no_grad():
                for _, (query_ids, query_seqs, target_ids, target_seqs, target_locs, labels) in tqdm(pairs.iloc[:, :].iterrows(), total=len(pairs)):
                    if query_seqs == 'Not found':
                        f.write(f"{query_ids}\t{query_seqs}\t{target_ids}\t{target_seqs}\t{target_locs}\t{0}\n")
                    else:
                        n0=query_seqs
                        n1=target_seqs
                        ...
                        p = p.item()
                        f.write(f"{query_ids}\t{query_seqs}\t{target_ids}\t{target_seqs}\t{target_locs}\t{p}\n")
                        if p >= threshold:
                            pos_f.write(f"{query_ids}\t{query_seqs}\t{target_ids}\t{target_seqs}\t{target_locs}\t{p}\n")
